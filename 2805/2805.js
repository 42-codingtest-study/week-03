...
function solution(input) {
  let treeNum = input[0].split(" ")[0] * 1; // 나무 갯수
  let targetLength = input[0].split(" ")[1] * 1; // 목표 길이

  const treeArr = input[1] // 나무 배열
    .split(" ")
    .map((e) => e * 1) //숫자로 변환
    .sort((a, b) => a - b); // 정렬
  //[ 10, 15, 17, 20 ]
  let left, right, mid; // 이분탐색을 할건데 우리가 목표하는 것은 배열내의 것이 아닌 배열 최소치와 최대치 사이의 어딘가 값인것을 고려해야한다.
